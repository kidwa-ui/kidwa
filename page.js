...
export default function Home() {
  ...
  const loadUserVotes = async () => {
    ...
    if (data) {
      const votesMap = {}
      data.forEach(v => {
        votesMap[v.poll_id] = { optionId: v.option_id, confidence: v.confidence }
      })
  ...
  const handleAuth = async (e) => {
    ...
    if (existingUser) {
      ...
      // Create new user
      const { data: newUser, error } = await createUser(username)
      if (newUser) {
        setUser(newUser)
  ...
  const handleVote = async (pollId, optionId, confidence = 50) => {
    ...
    const { data, error } = await vote(user.id, pollId, optionId, confidence)
    if (!error) {
      setUserVotes(prev => ({
        ...prev,
        [pollId]: { optionId, confidence }
      }))
      loadPolls() // Reload to get updated votes
    }
  }
  ...
  const filteredPolls = polls.filter(poll => {
    ...
    if (searchQuery) {
      ...
      return poll.question.toLowerCase().includes(q) || 
             poll.tags?.some(t => t.name.toLowerCase().includes(q))
    }
    return true
  })
            ...
            <>
              ...
              <section>
                <h2 className="section-title">
                  {activeCategory === 'home' ? '🆕 ล่าสุด' : `${categories.find(c => c.id === activeCategory)?.icon} ${categories.find(c => c.id === activeCategory)?.name}`}
                </h2>
                <div className="poll-grid">
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleAuth}>
              ...
              <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '1rem' }}>
                🎁 สมัครใหม่ได้ 1,000 Reputation เริ่มต้น!
              </p>
      ...
      {selectedPoll && (
        <div className="modal-overlay" onClick={() => setSelectedPoll(null)}>
          <div className="modal" style={{ maxWidth: '550px' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {selectedPoll.options?.map(option => {
                ...
                return (
                  <button
                    key={option.id}
                    onClick={() => handleVote(selectedPoll.id, option.id, 50)}
                    style={{
                      position: 'relative',
                      padding: '1rem',
                      border: `2px solid ${isSelected ? 'var(--primary)' : 'var(--border)'}`,
                      borderRadius: '10px',
                      background: isSelected ? 'rgba(255, 107, 157, 0.1)' : 'var(--bg)',
                      cursor: 'pointer',
                      textAlign: 'left',
                      fontFamily: 'inherit',
                      overflow: 'hidden'
            ...
            {!user && (
              <div 
                onClick={() => { setSelectedPoll(null); setShowAuthModal(true); }}
                style={{
                  marginTop: '1rem',
                  padding: '0.75rem',
                  background: 'var(--bg)',
